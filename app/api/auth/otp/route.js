import { ReS, ReE } from "@/app/_utils/responseHandler.util";
import OTP from "@/app/_schemas/otp.schema";
import { hashSync } from "bcrypt";
import { sendMail } from "@/app/_lib/mail/sendMail";
import getOTP from "@/app/_utils/getOTP";
import getTemplateData from "@/app/_lib/mail/templates/getTemplateData";
import dbConnect from "@/app/_lib/dbConnect";
import User from "@/app/_schemas/user.schema";

export async function POST(request) {
  try {
    dbConnect();
    const { email, type } = await request.json();

    if(type === "signup"){
      const userExist = await User.findOne({ email });
      if(userExist){
        return ReE("User already exist", "", 400);
      }
    }
    else if(type === "resetPassword"){
      const userExist = await User.findOne({ email });
      if(!userExist){
        return ReE("User not exist", "", 400);
      }
    }

    if (!email) {
      return ReE("Missing required fields", "email is required", 400);
    }

    const otp = getOTP(); // 6-digit OTP
    console.log(otp)
    const otpHash = hashSync(otp, parseInt(process.env.SALT_ROUND));

    const expiresAt = new Date(Date.now() + 5 * 60 * 1000); // 5 minutes from now

    await OTP.create({
      target: email,
      type: type,
      otpHash,
      expiresAt,
    });

    // Uncomment this if you want to check for recent OTPs before sending a new one
    // const recentOtp = await OTP.findOne({ target: email, type }).sort({ createdAt: -1 });
    // if (recentOtp && recentOtp.expiresAt > new Date()) 
    //   return ReE("Please wait before requesting another OTP", "", 429);

    const mail = await sendMail({
      to: email,
      data: getTemplateData(type, otp),
    });

    if(!mail.success) {
      return ReE("Failed to send OTP", mail.error, 500);
    }

    return ReS("OTP sent successfully", [], 200);
  } catch (error) {
    console.error("Error sending OTP:", error);
    return ReE("Failed to send OTP", error, 500);
  }
}
